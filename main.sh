...
if [ -f "$SCRIPT_PATH/includes/banners.sh" ]; then
    source "$SCRIPT_PATH/includes/banners.sh"
else
    echo "[ERROR] The $SCRIPT_PATH/includes/banners.sh\" file is missing."
    exit 1
fi
...
function print_banner {
    echo
    RANDOM_INDEX=$(( ( RANDOM % ${#AVAILABLE_BANNERS[@]} ) ))
    # echo "Random index: ${RANDOM_INDEX}"
    echo -e "${AVAILABLE_BANNERS[$RANDOM_INDEX]}"
}
