...
SCRIPT_NAME='Install Inkscape from official PPA'
SCRIPT_VERSION='1.0'
...
function _run() {
    echo
    echo -e " ${CYAN}[ Installing Inkscape from official PPA ]${NC}"
    write_log "\n\n[ Installing Inkscape from official PPA ]"

    write_log ''
    msg='Adding PPA repository'
    write_log "[ CURRENT STEP ]: ${msg}"
    printf "${NC}\t%s %s [...]" "${msg}" "${PADD_LINE:${#msg}}"
    if [ $SIMULATION_MODE == true ]; then
        status=$(echo -e ${DARK_GREEN}"${DONE_SYMBOL}"${NC})
    else
        status=$(add-apt-repository -y ppa:inkscape.dev/stable 2>&1 | tee -a $LOG_FILE_PATH >/dev/null && echo -e ${DARK_GREEN}"${DONE_SYMBOL}"${NC} || echo -e ${RED}"${FAILED_SYMBOL}"${NC})
    fi
    ...
    if [ $SIMULATION_MODE == true ]; then
        ...
        status=$((apt update && apt install -y inkscape) 2>&1 | tee -a $LOG_FILE_PATH >/dev/null && echo -e ${DARK_GREEN}"${DONE_SYMBOL}"${NC} || echo -e ${RED}"${FAILED_SYMBOL}"${NC})
    fi
    printf "\r${NC}\t%s %s [%s]${CLEAR_REST_OF_LINE}\n" "${msg}" "${PADD_LINE:${#msg}}" "${status}"
